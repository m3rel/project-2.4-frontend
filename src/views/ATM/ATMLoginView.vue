<template>
  <div class="h-screen w-screen gradient-bg-atm flex items-center justify-center">
    <div class="flex flex-col items-center">
      <h1 class="text-7xl font-serif">ATM</h1>
...
const handleLogin = async () => {
  try {
    ...
    if (response.data.role !== 'ROLE_CUSTOMER') {
      error.value = 'Only customers can use the ATM'
      return
    }

    authStore.setAuth(response.data.token, response.data.role)
    router.push('/atm/machine')
